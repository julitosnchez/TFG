...
def selection_and_reproduction(gen,population,q_d):
    ...
    if gen == 0:
     ...
     error_ant = np.load(os.path.join('ROMBO','error'+str(gen-1) + '.npy'));
     gen_ant = np.load(os.path.join('ROMBO','generacion'+str(gen-1) + '.npy'));
    ...
    np.save(os.path.join('ROMBO','error' + str(gen)+'.npy'),error);

#    pdb.set_trace()
    puntuados = [j[1] for j in sorted(puntuados)[::-1]] ;#Ordena los pares ordenados y se queda solo con el array de valores
    population = puntuados;
  
    np.save(os.path.join('ROMBO','generacion'+str(gen)),np.array(population));
#    pdb.set_trace()
...
def generaMovimientoHorizontal(n,limb):
    ...
    puntos = [];
    puntos.append(qDes1); puntos.append(qDes2);
...
if __name__ == "__main__":
   ...
   n = 300000
   qObjs = generaMovimientoHorizontal(n,limb)

   population = np.load('generacion600.npy');
   calcularFitness(population[9],qObjs);
   #Se evoluciona la poblacion
   #i = 0
   #while i < numero_generaciones:
   #  population = selection_and_reproduction(i,population,qObjs)
